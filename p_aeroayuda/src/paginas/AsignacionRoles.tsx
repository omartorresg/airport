...
const AsignacionRolesEquipos = () => {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      const { data: personal } = await supabase.from('personal_operativo').select('id_personal, nombre, departamentos(nombre)');
  ...
  const asignar = async () => {
    ...
    const { error } = await supabase.from('asignacion_roles_equipos').insert([
      { id_personal: parseInt(idPersonal), id_rol: parseInt(idRol), id_equipo: parseInt(idEquipo) }
    ]);
      ...
      <div className="contenedor">
        <div className="contenedor-asignacion">
          ...
          <select className="input-asignacion" value={idPersonal} onChange={(e) => setIdPersonal(e.target.value)}>
            ...
            {personalDB.map(p => (
              <option key={p.id_personal} value={p.id_personal}>{`${p.nombre} - ${p.departamento}`}</option>
            ))}
          </select>
